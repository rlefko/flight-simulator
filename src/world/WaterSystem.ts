...
export class WaterSystem {
        ...
        for (let i = 0; i < resolution; i++) {
            for (let j = 0; j < resolution; j++) {
                ...
                if (waterMask[index] && !visited.has(index)) {
                    const waterRegion = this.floodFillWater(
                        waterMask, visited, j, i, resolution
                    );
                    
                    if (waterRegion.length > 10) { // Minimum size threshold
                        const waterBody = this.createWaterSurfaceFromRegion(
                            tile, waterRegion, heightmap, resolution
                        );
    ...
    private calculateShoreData(x: number, z: number): ShoreData {
        ...
        const foamIntensity = isShore ? Math.max(0, 1 - nearestShoreDistance / WATER_CONFIG.SHORE.foamWidth) : 0;
