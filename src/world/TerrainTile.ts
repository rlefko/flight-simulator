...
import { TERRAIN_CONFIG, LOD_CONFIG, getTileSizeForLOD, getMeshResolutionForLOD } from './WorldConstants';
...
export class TerrainTile {
    ...
    public shouldSubdivide(cameraPosition: Vector3, errorThreshold: number): boolean {
        ...
        const subdivisionDistance = LOD_CONFIG.TERRAIN_DISTANCES[this.level + 1] || LOD_CONFIG.CULL_DISTANCE;
    ...
    public isReadyForRender(): boolean {
        return this.state === TerrainTileState.READY && 
               this.meshData !== null && 
    ...
    public calculateError(): number {
        ...
        this.error = (elevationRange + geometricComplexity) / Math.max(1, this.distanceToCamera / 1000);
    ...
    public containsPoint(x: number, z: number): boolean {
        return x >= this.worldBounds.minX && 
    ...
    public serialize(): any {
        return {
            ...
            terrainData: this.terrainData ? {
                ...
                normals: this.terrainData.normals ? Array.from(this.terrainData.normals) : null,
                materials: this.terrainData.materials ? Array.from(this.terrainData.materials) : null,
                waterMask: this.terrainData.waterMask ? Array.from(this.terrainData.waterMask) : null
            } : null,
    ...
    public static deserialize(data: any): TerrainTile {
        ...
        if (data.terrainData) {
            tile.terrainData = {
                ...
                normals: data.terrainData.normals ? new Float32Array(data.terrainData.normals) : undefined,
                materials: data.terrainData.materials ? new Uint8Array(data.terrainData.materials) : undefined,
                waterMask: data.terrainData.waterMask ? new Uint8Array(data.terrainData.waterMask) : undefined
            };
