...
export class HeightmapGenerator {
    ...
    private perlinNoise(x: number, z: number): number {
        ...
        const x1 = this.lerp(
            this.grad(aa, xf, zf),
            this.grad(ba, xf - 1, zf),
            u
        );
        
        const x2 = this.lerp(
            this.grad(ab, xf, zf - 1),
            this.grad(bb, xf - 1, zf - 1),
            u
        );
    ...
    private applyErosion(heightmap: Float32Array, size: number, params: ErosionParams): void {
        for (let iteration = 0; iteration < params.iterations; iteration++) {
            ...
            for (let lifetime = 0; lifetime < params.dropletLifetime; lifetime++) {
                ...
                const capacity = Math.max(-heightDelta * speed * water * params.sedimentCapacityFactor, params.minSedimentCapacity);
                ...
                if (sediment > capacity || heightDelta > 0) {
                    ...
                    this.depositSediment(heightmap, size, nodeX, nodeZ, amountToDeposit, params.brushRadius);
                } else {
                    // Erode
                    const amountToErode = Math.min((capacity - sediment) * params.erodeSpeed, -heightDelta);
                    
                    // Erode in a brush pattern
                    this.erodeSediment(heightmap, size, nodeX, nodeZ, amountToErode, params.brushRadius);
    ...
    private generateClimate(x: number, z: number, elevation: number): ClimateData {
        ...
        const humidity = Math.min(1, precipitation / 2000) * Math.max(0, 1 - Math.abs(temperature - 20) / 40);
    ...
    private getHeightInterpolated(heightmap: Float32Array, size: number, x: number, z: number): number {
